...
const OwnerCrown = (() => {
    ...
    return class {
        ...
        start() {
            const ServerMembers = BdApi.findModuleByProps("getMember");
            const UserStore = BdApi.findModuleByProps("getUser");

            BdApi.Patcher.after("OwnerCrown", ServerMembers, "getMember", (_, args, returnValue) => {
                const guildId = args[0];
                const ownerId = ServerMembers.getOwnerID(guildId);
                const memberId = args[1];
                const member = returnValue;
